...
public final class TVertexScalerWidget {
	...
	public ScaleDirection getDirectionByMouse(final Point mousePoint, final CoordinateSystem coordinateSystem, final byte dim1, final byte dim2) {
		...
		if (((Math.abs(x - mousePoint.getX()) <= 4)
				&& (mousePoint.y < y)
				&& (mousePoint.y > (y - LINE_LEN)))) {
			direction = ScaleDirection.UP;
		}
		if (((Math.abs(y - mousePoint.getY()) <= 4)
				&& (mousePoint.x > x)
				&& (mousePoint.x < (x + LINE_LEN)))) {
		...
		if (moveDirection != null) {
			switch (moveDirection) {
				case XYZ -> {
					...
					graphics.drawLine((int) x + EXTERIOR_TRIANGLE_OFFSET, (int) y, (int) x, (int) y - EXTERIOR_TRIANGLE_OFFSET);
				...
				case UP -> {
					setColorByDimension(graphics, xDimension);
					graphics.drawLine((int) x + EXTERIOR_TRIANGLE_OFFSET, (int) y, (int) x + (EXTERIOR_TRIANGLE_OFFSET / 2), (int) y - (EXTERIOR_TRIANGLE_OFFSET / 2));
					graphics.drawLine((int) x + INTERIOR_TRIANGLE_OFFSET, (int) y, (int) x + (INTERIOR_TRIANGLE_OFFSET / 2), (int) y - (INTERIOR_TRIANGLE_OFFSET / 2));
					graphics.drawLine((int) x, (int) y, (int) x + LINE_LEN, (int) y);
					graphics.drawRect(((int) x + LINE_LEN) - 2, (int) y - 2, 4, 4);
					...
					setColorByDimension(graphics, yDimension);
					graphics.drawLine((int) x + (EXTERIOR_TRIANGLE_OFFSET / 2), (int) y - (EXTERIOR_TRIANGLE_OFFSET / 2), (int) x, (int) y - EXTERIOR_TRIANGLE_OFFSET);
					graphics.drawLine((int) x + (INTERIOR_TRIANGLE_OFFSET / 2), (int) y - (INTERIOR_TRIANGLE_OFFSET / 2), (int) x, (int) y - INTERIOR_TRIANGLE_OFFSET);
					graphics.drawRect((int) x - 2, (int) y - LINE_LEN - 2, 4, 4);
				}
				case RIGHT -> {
					graphics.setColor(new Color(255, 255, 0));
