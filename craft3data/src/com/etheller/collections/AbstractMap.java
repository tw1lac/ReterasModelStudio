 ...
 * returns a set-view of the map's mappings. Typically, the returned set will,
 * in turn, be implemented atop <tt>AbstractSet</tt>. This set should not
 * support the <tt>add</tt> or <tt>remove</tt> methods, and its iterator should
 * not support the <tt>remove</tt> method.
 *
 * <p>
 * To implement a modifiable map, the programmer must additionally override this
 * class's <tt>put</tt> method (which otherwise throws an
...
public abstract class AbstractMap<K, V> implements Map<K, V> {
	 ...
	 *           <tt>put</tt> operation once for each entry returned by the
	 *           iteration.
	 *
	 *           <p>
	 *           Note that this implementation throws an
	 *           <tt>UnsupportedOperationException</tt> if this map does not
	 *           support the <tt>put</tt> operation and the specified map is
	 *           nonempty.
	 *
	 * @throws UnsupportedOperationException
	 *             {@inheritDoc}
	...
	public Set<K> keySet() {
		if (keySet == null) {
			keySet = new AbstractSet<K>() {
				private KeySetForEachAdapter forEachAdapter;
				...
				@Override
				public void forEach(final CollectionView.ForEach<? super K> forEach) {
					if (forEachAdapter == null) {
						forEachAdapter = new KeySetForEachAdapter(forEach);
					}
					AbstractMap.this.forEach(forEachAdapter);
				}
			};
		}
		return keySet;
	...
	public Collection<V> values() {
		if (values == null) {
			values = new AbstractCollection<V>() {
				...
				@Override
				public void forEach(final com.etheller.collections.CollectionView.ForEach<? super V> forEach) {
	...
	protected Object clone() throws CloneNotSupportedException {
		final AbstractMap<?, ?> result = (AbstractMap<?, ?>) super.clone();
		result.keySet = null;
		result.values = null;
		return result;
	}
	...
	private final class KeySetForEachAdapter implements MapView.ForEach<K, V> {
		private final CollectionView.ForEach<? super K> forEach;

		private KeySetForEachAdapter(final CollectionView.ForEach<? super K> forEach) {
			this.forEach = forEach;
		}

		@Override
		public boolean onEntry(final K key, final V value) {
			return forEach.onEntry(key);
		}
	}

	private final class ValuesForEachAdapter implements MapView.ForEach<K, V> {
		private final CollectionView.ForEach<? super V> forEach;

		private ValuesForEachAdapter(final CollectionView.ForEach<? super V> forEach) {
			this.forEach = forEach;
		}

		@Override
		public boolean onEntry(final K key, final V value) {
			return forEach.onEntry(value);
		}
	}
