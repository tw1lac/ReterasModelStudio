...
import com.hiveworkshop.wc3.mdl.render3d.*;
...
//				layer.getFilterMode() == FilterMode.ADDITIVE, layer.getFilterMode() == FilterMode.ADDITIVE);
	...
	public double convertX(final double x) {
		return ((x + cameraPos.x) * m_zoom) + ((double) getWidth() / 2);
	}

	public double convertY(final double y) {
		return ((-y + cameraPos.y) * m_zoom) + ((double) getHeight() / 2);
	}

	public double geomX(final double x) {
		return ((x - ((double) getWidth() / 2)) / m_zoom) - cameraPos.x;
	}

	public double geomY(final double y) {
		return -(((y - ((double) getHeight() / 2)) / m_zoom) - cameraPos.y);
	}
