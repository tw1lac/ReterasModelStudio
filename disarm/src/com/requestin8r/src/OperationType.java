	...
	static {
		add = new OperationGroup("Add", "Add something to the model.", IconGet.get("Replay-SpeedUp", 64))
			.add(new OperationType("Shape", "Add a 3D shape from something else to the model.", IconGet.get("HumanArmorUpOne", 64))
				.addField(new Field<>("Source Model", "The model from which to get the shape to add to this model.", String.class))
				.addField(new Field<>("Which Part", "The part of the source model to add.", String.class))
				.addField(new Field<>("Attach to", "The part of the this model to attach the shape onto.", Bone.class))
				.addField(new Field<>("Scale", "The size of the part once placed into this model.", Float.class))
				)
			.add(new OperationGroup("Player Color", "Add more to the model that changes color with ownership.", IconGet.get("OrcCaptureFlag", 64))
					...
					.add(new OperationType("Custom Texture", "Add player color marks on the texture of this model, thereby creating a custom texture.", IconGet.get("Spy", 64))
						.addField(new Field<>("Where?", "What part of the texture should gain player color markings?", String.class))
						.addField(new Field<>("New Texture", "Make a rough sketch of what the new texture should look like.", Image.class))
						)
				)
			.add(new OperationType("Glow", "Add a glow with a constant shape to the model.", new ImageIcon(RemovePanel.class.getResource("BTNTeamGlow00.png")).getImage())
				.addField(new Field<>("Glow Texture", "The texture to use for the glow.", String.class))
				.addField(new Field<>("Attach to", "The part of the this model to attach the glow onto.", Bone.class))
			)
			.add(new OperationType("Attachment", "Add an attachment point to the model.", IconGet.get("Glove", 64))
				.addField(new Field<>("Name", "The name, or type, of attachment to add.", String.class))
				.addField(new Field<>("Attach to", "What to attach the new attachment point onto within the model.", Bone.class))
			)
			.add(new OperationGroup("Special Effect", "A special effect in the model, such as a particle emitter or glow.", IconGet.get("ScatterRockets", 64))
				.add(new OperationType("Particle Emitter", "Add a particle emitter.", IconGet.get("Fire", 64))
					.addField(new Field<>("Style", "The style of particle -- fire, magic, clouds, or custom", String.class))
					.addField(new Field<>("Color", "The emitter's color.", Color.class))
					.addField(new Field<>("Attach to", "What to attach the new particle emitter onto within the model.", Bone.class))
				)
				.add(new OperationType("Ribbon Emitter", "Add a ribbon emitter.", IconGet.get("AntiMagicShell", 64))
					.addField(new Field<>("Texture", "The texture to use for creating a ribbon", String.class))
					.addField(new Field<>("Color", "The emitter's color.", Color.class))
					.addField(new Field<>("Attach to", "What to attach the new ribbon emitter onto within the model.", Bone.class))
				)
				.add(new OperationType("Light", "Add a light emitter.", IconGet.get("HolyBolt", 64))
					.addField(new Field<>("Color", "The emitter's color.", Color.class))
					.addField(new Field<>("Intensity", "The emitter's brightness level.", Integer.class))
					.addField(new Field<>("Attach to", "What to attach the new light emitter onto within the model.", Bone.class))
				)
				.add(new OperationType("Sound Effect", "Add a sound effect.", IconGet.get("CallToArms", 64))
					.addField(new Field<>("Type", "What type of sound emitter?", String.class))
					.addField(new Field<>("Attach to", "What to attach the new sound emitter onto within the model.", Bone.class))
				)
			)
			;
	}
