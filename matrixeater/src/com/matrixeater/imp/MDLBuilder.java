...
public class MDLBuilder implements BuilderInterface {

    private final Logger log = Logger.getLogger(MDLBuilder.class.getName());
	...
	public void addVertexGeometric(final float x, final float y, final float z) {
        verticesG.add(new Vertex(x, y, z));
...
//        log.log(INFO,"Added texture  vertex " + verticesT.size() + " = " + verticesT.get(verticesT.size() - 1));
	...
	public void addVertexNormal(final float x, final float y, final float z) {
        verticesN.add(new Normal(x, y, z));
    }
...
//            log.log(INFO,"Adding vertex g=" + vertexIndices[loopi] + " t=" + vertexIndices[loopi + 1] + " n=" + vertexIndices[loopi + 2]);
            ...
            if (((vertexIndex - 1) >= 0) && ((vertexIndex - 1) < verticesG.size())) {
                ...
                //fv.v = verticesG.get(vertexIndex - 1); TODO broken line
            } else {
            ...
            if (vertexIndex != EMPTY_VERTEX_VALUE) {
                ...
                if (((vertexIndex - 1) >= 0) && ((vertexIndex - 1) < verticesT.size())) {
                    // Note: vertex indices are 1-indexed, i.e. they start at
                    // one, so we offset by -1 for the 0-indexed array lists.
                    //fv.t = verticesT.get(vertexIndex - 1); TODO broken line
                } else {
            ...
            if (vertexIndex != EMPTY_VERTEX_VALUE) {
                ...
                if (((vertexIndex - 1) >= 0) && ((vertexIndex - 1) < verticesN.size())) {
                    // Note: vertex indices are 1-indexed, i.e. they start at
                    // one, so we offset by -1 for the 0-indexed array lists.
                    //fv.n = verticesN.get(vertexIndex - 1); TODO broken line
                } else {
            ...
            if (null == fv2) {
                ...
                //fv.index = faceVerticeList.size(); TODO broken line
